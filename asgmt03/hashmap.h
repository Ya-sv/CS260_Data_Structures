...
private:
	...
	enum  positionInfo {EMPTY, OCCUPIED, DELETED }; // track every slots {empty, occupied and deleted}

	// Use this struct for each slot in the HashMap's underlying array.
	// You can add new members to this struct if you need to.
	struct Slot
	{
		Slot(void) :slotStock{ Stock() },slotInfoState(EMPTY) { }
		Stock	slotStock;
		positionInfo slotInfoState;
	};
